...
interface BacktestChartProps {
    ...
    fastMaData: LightweightCharts.LineData[];
    slowMaData: LightweightCharts.LineData[];
    signals: LightweightCharts.SeriesMarker<LightweightCharts.Time>[];
    fastMaPeriod: number;
    slowMaPeriod: number;
}
...
const BacktestChart: React.FC<BacktestChartProps> = ({ data, fastMaData, slowMaData, signals, fastMaPeriod, slowMaPeriod }) => {
  ...
  useEffect(() => {
    ...
    const formattedData = data.map(d => ({...d, time: d.time as LightweightCharts.Time}));
    candlestickSeries.setData(formattedData);
    candlestickSeries.setMarkers(signals);

    const fastMaSeries = chart.addLineSeries({ color: '#1F6FEB', lineWidth: 2, priceLineVisible: false, lastValueVisible: false });
    fastMaSeries.setData(fastMaData);

    const slowMaSeries = chart.addLineSeries({ color: '#F7B93E', lineWidth: 2, priceLineVisible: false, lastValueVisible: false });
